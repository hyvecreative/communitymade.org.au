	...
	if ($installed_version != $actionnetwork_version) {
		...
		if ($actionnetwork_version == '1.4.3') {
			$notices[] = sprintf(
				...
				__('The %s has been updated to version 1.4.4.', 'actionnetwork'),
		...
		if ($actionnetwork_version == '1.4.2') {
			$notices[] = sprintf(
				/* translators: %s is a link to https://wordpress.org/plugins/wp-action-network/ */
				__('The %s has been updated to version 1.4.3.', 'actionnetwork'),
				'<a href="https://wordpress.org/plugins/wp-action-network/">Action Network plugin</a>'
				);
		}
		...
		if ($actionnetwork_version == '1.4.1') {
			$notices[] = sprintf(
				/* translators: %s is a link to https://wordpress.org/plugins/wp-action-network/ */
				__('The %s has been updated to version 1.4.2. This update is a quick bug fix to support Action Network URLs up to version 4.', 'actionnetwork'),
				'<a href="https://wordpress.org/plugins/wp-action-network/">Action Network plugin</a>'
				);
		}
		...
		if ($actionnetwork_version == '1.4.1') {
			$notices[] = sprintf(
				/* translators: %s is a link to https://wordpress.org/plugins/wp-action-network/ */
				__('The %s has been updated to version 1.4.1. This update fixes a bug with widget hCaptcha protection when hCaptcha keys are not provided. The hCaptcha verification will require both hCaptcha keys in the plugin settings and the "Spam protection" checkbox be enabled. If either are not provided, hCaptcha protection will be disabled, allowing the form to be submitted without verification.', 'actionnetwork'),
				'<a href="https://wordpress.org/plugins/wp-action-network/">Action Network plugin</a>'
				);
		}
...
EOHTML;
	...
	if ($n) { $sql .= " LIMIT 0,$n"; }
	$actions = $wpdb->get_results( $wpdb->prepare($sql), ARRAY_A );
...
EOHTML;
	...
	if ($n) { $sql .= " LIMIT 0,$n"; }
	$events = $wpdb->get_results( $wpdb->prepare($sql), ARRAY_A );
...
function actionnetwork_cron_sync() {
	...
	$timeint = time() / wp_rand( 1, 10 ) * wp_rand( 1, 10 );
		...
		<div class="wrap-inner">
			...
			<div class="actionnetwork-admin-tab<?php echo ($tab == 'actions') ? ' actionnetwork-admin-tab-active' : ''; ?>" id="actionnetwork-actions">
				<h2>
					...
					<?php if (isset($_REQUEST['search']) && $_REQUEST['search']) {
						...
						printf( __('Search results for test "%s"', 'actionnetwork'),  stripslashes(esc_attr($_REQUEST['search'])) );
						echo '</span>';
					} ?>
				</h2>
				...
				<form id="actionnetwork-actions-filter" method="get">
					...
					<p class="search-box">
						...
						<input type="search" id="action-search-input" name="search" value="<?php echo isset($_REQUEST['search']) ? stripslashes(esc_attr($_REQUEST['search'])) : '' ?>" placeholder="<?php _e('Search','actionnetwork'); ?>" />
			...
			<div class="actionnetwork-admin-tab<?php echo ($tab == 'cron-updates') ? ' actionnetwork-admin-tab-active' : ''; ?>" id="actionnetwork-cron-updates">
					...
					if($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $table_name)) == $table_name):
						$user = $wpdb->get_results( $wpdb->prepare("SELECT * FROM %i", $table_name) );
						if(count($user)!=0):
					?>
