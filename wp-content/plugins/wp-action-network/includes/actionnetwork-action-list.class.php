...
*/
	...
	function get_actions( $per_page = 20, $page_number = 1 ) {
		...
		if ( !empty( $_REQUEST['orderby'] ) ) {
			$orderby = (in_array(strtolower($_REQUEST['orderby']), array('modified_date', 'title', 'type')) ? $_REQUEST['orderby'] : 'modified_date');
			$order = (!empty( $_REQUEST['order'] ) && in_array(strtolower($_REQUEST['order']), array('asc', 'desc')) ? $_REQUEST['order'] : 'ASC');
	...
	function extra_tablenav( $which ) {
		...
		if ( $which == 'top' ) {
			...
			foreach ($this->action_type_plurals as $key => $plural) {
				$sql = $wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}actionnetwork WHERE type= %s", $key);
