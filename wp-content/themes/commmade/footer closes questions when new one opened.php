...
<script>
        ...
        document.querySelectorAll('.schema-faq-question').forEach(q => {
          q.addEventListener('click', () => {
            const allAnswers = document.querySelectorAll('.schema-faq-answer');
            const answer = q.nextElementSibling;

            // Close all other answers
            allAnswers.forEach(a => {
              if (a !== answer && a.style.height && a.style.height !== '0px') {
                a.style.height = a.scrollHeight + 'px'; // Ensure current height
                requestAnimationFrame(() => {           // Allow browser to register height
                  a.style.height = '0';
                  a.style.opacity = 0;
                });
              }
            ...
            if (answer.style.height && answer.style.height !== '0px') {
              ...
              // Expand
              answer.style.height = '0'; // Ensure start from 0
              answer.style.opacity = 0;
              requestAnimationFrame(() => {
